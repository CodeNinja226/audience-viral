...
func (insta *Instagram) sendRequest(o *reqOptions) (body []byte, err error) {
	...
	if msg.Challenge != nil && msg.Challenge.Path != "" {
		...
		insta.Challenge.Path = strings.Replace(msg.Challenge.Path, "/challenge/", "challenge/", 1)
	}

	if msg.TwoFactorRequired {
		insta.ChallengeReq = false
		insta.TwoFactorReq = true
		return body, ErrTwoFactorAuthRequired
	}
	...
	case "select_verify_method", "verify_code", "submit_phone", "verify_email":
		needsChallenge = true
		insta.TwoFactorReq = false
		insta.ChallengeReq = true
	}
