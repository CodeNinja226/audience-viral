...
import (
	...
	"github.com/abramovic/goinsta/response"
)
...
const (
	...
	GOINSTA_CHALLENGE_SMS   string = "0"
	GOINSTA_CHALLENGE_EMAIL string = "1"
	NoCSRFToken                    = "None"
)
...
func New(username, password string) *Instagram {
	...
	cookieJar, _ := cookiejar.New(nil) //newJar()
...
// TwoFactorLogin will login to instagram using two-factor authorization
func (insta *Instagram) TwoFactorLogin(identifier, userCode string) error {
	// ensure that the verification code is clean
	userCode = strings.Replace(userCode, " ", "", -1)
	userCode = strings.Replace(userCode, "-", "", -1)
	userCode = strings.Replace(userCode, ".", "", -1)
	userCode = strings.Replace(userCode, "_", "", -1)

	if len(userCode) != 6 {
		return fmt.Errorf("Invalid two-factor code: %v", userCode)
	}
...
func (insta *Instagram) Login() error {
	...
	// try to see if two-factor authorization is required
	if err != nil && err == ErrTwoFactorAuthRequired {
		var msg apiResponseMessage
		json.Unmarshal(body, &msg)
		if msg.TwoFactorInfo != nil {
			insta.TwoFactorReq = true
			insta.TwoFactor.ID = msg.TwoFactorInfo.TwoFactorIdentifier
			return err
		}
	}
	if err != nil {
	...
	if err := insta.login(body); err != nil {
		return err
	}
	return nil
}

func (insta *Instagram) login(body []byte) error {
...
func (insta *Instagram) RequestChallengeCode() (body []byte, err error) {
	var v = url.Values{
		"choice": {string(insta.Challenge.Choice())},
	}
	return insta.sendRequest(&reqOptions{
		Endpoint:    insta.Challenge.Path,
		PostData:    string(v.Encode()),
		IsLoggedIn:  true,
		IsChallenge: true,
	})
...
func (insta *Instagram) SubmitChallengeCode() (body []byte, err error) {
	...
	var v = url.Values{
		"security_code":       {insta.Challenge.Code},
		"guid":                {insta.Informations.UUID},
		"login_attempt_count": {"0"},
		"_csrftoken":          {insta.Informations.Token},
		"device_id":           {insta.Informations.DeviceID},
